...
```
Projeto → Participantes → Tasks → Workspace (Pod + Clone) → Git Branch
```
...
- **Uma Task pertence a um projeto** e possui um autor responsável (`assignedTo`), uma branch associada (ex: `task/introducao-123`), um status do workflow (`NOT_STARTED`, `IN_PROGRESS`, `UNDER_REVIEW`, `CHANGES_REQUESTED`, `APPROVED`, `MERGED`) e opcionalmente um `PullRequest`.
...
- Card com título da tarefa, prazos e status.
...
| :--- | :--- | :--- |
| **AUTOR (`AUTHOR`)** | Clica em `[Continuar Trabalho]` na sua Task | Abre o editor na branch `task/*` para escrita LaTeX. Botão de *Salvar Progresso* e *Enviar p/ Revisão*. |
