...
5. **Gerenciamento Dinâmico de Pods por Presença SSE com Grace Period de 3 Minutos (`releasePodForProject`):**
   - **Problema:** Pods do `code-server` continuavam rodando indefinidamente no cluster K8s consumindo memória RAM/CPU mesmo após o usuário fechar a aba ou sair do projeto, ou eram destruídos prematuramente durante oscilações do SSE.
   - **Solução:** Integrado o monitoramento de conexões SSE (`EventsManagerService`) rastreado por `projectId`. Quando as conexões SSE ativas do projeto chegam a `0`, o backend aciona um **Grace Period de 3 minutos (`180.000 ms`)**. Se o usuário reconectar/der F5 dentro deste intervalo, o timer é cancelado e o Pod permanece ativo. Se o tempo expirar sem nova conexão, o backend executa `releasePodForProject(projectId)`, deletando o Pod no Kubernetes e liberando a memória RAM.

6. **Isolamento Estrito de Projetos, Roteamento Dinâmico no Service K8s e Reverse Proxy Fastify:**
   - **Problema:** A presença do rótulo genérico `component=code-server` em pods reservas fazia o Service `code-server-service` (NodePort 30080) rotacionar o tráfego entre pods aleatórios, fazendo o VS Code Web carregar pastas erradas ou em branco. Além disso, a exposição direta do NodePort no navegador apresentava riscos de segurança.
   - **Solução:** 
     1. **Isolamento de Rótulos K8s:** Os pods de reserva usam o rótulo `component: code-server-warm`.
     2. **Seletor Dinâmico por Projeto:** Ao reivindicar/criar um Pod, o `K8sPodManagerService` atualiza dinamicamente o `spec.selector` do `code-server-service` para `projectId: <projectId>` e executa *readiness polling* (`waitForPodReady`) até o container ficar 100% pronto.
     3. **Reverse Proxy Seguro (@fastify/http-proxy):** A rota `/api/v1/editor-proxy/app` faz o proxy seguro do tráfego HTTP e WebSockets do backend Fastify diretamente para a porta interna do container, fechando a exposição pública da porta NodePort 30080.
     4. **SubPath Mount & Pré-provisionamento:** Os Pods declaram `subPath: projects/${projectId}/users/${userId}` montado em `/home/coder/project` e os modelos TeX (`main.tex`, `sections/`, `IEEEtran.cls`) são gerados com permissões `0o777` antes do redirecionamento.
     5. **Rastreabilidade na Tabela `Workspace`:** A sessão ativa do Pod é gravada na tabela `Workspace` do banco de dados Prisma (`projectId`, `userId`, `podName`, `status`), permitindo consulta de ocupação do workspace em tempo real.
